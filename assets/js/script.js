$(document).ready(function (){
    ...
    function cityAllStorage (){
        ...
        for (i=0; i<localCities.length; i++){
            ...
            $(".city-list").prepend("<button type='button' class='btn btn-info btn-block'>"+ localCities[i]+ "</button>");
        }
    ...
    $(".myClass").on('click', function (event){
        ...
        if (city != ''){
            $.ajax({
                url: `https://api.openweathermap.org/data/2.5/weather?q=${city}&units=imperial&APPID=b82beadaf2428e27794315f09f4f6b24`,
                type: "GET",
                datatype: "jsonp",
            ...
            $(".city-list").append("<button class='btn btn-info btn-block'>"+ cities + "</button>")
    ...
    function uvIndexFunction(lat, lon) {
        ...
        var queryURLData = `https://api.openweathermap.org/data/2.5/weather?lat=${lat}&lon=${lon}&exlclude={part}&APPID=b82beadaf2428e27794315f09f4f6b24`
        $.ajax({
            url: queryURLData,
            method: "GET"
        }).then(function (data) {
...
// current weather container summary function to print on HTML div
    ...
    function fiveDayForecast (city) {
        $.ajax ({
            url: `https://api.openweathermap.org/data/2.5/forecast?q=${city}&units=imperial&appid=b82beadaf2428e27794315f09f4f6b24`,
            type: "GET",
            datatype: "jsonp",
